class StandartCheckException{
    ...
    public VoiceException(data: { phone_number: string; text: string, photo: string, file: string, stickers: string, video: string, location: number[], polls: string, voice: string, audio: string, video_circle: string }): boolean {
      if (data.phone_number === '' && data.photo === '' && 
      data.text === '' && data.file === '' && 
      data.video === '' && data.location[0] === -1 &&
      data.stickers === '' && data.polls === '' &&
